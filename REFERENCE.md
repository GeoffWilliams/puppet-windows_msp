...
* [`windows_msp`](#windows_msp): Install MSP patches using Puppet.  Since we don't know whats inside the MSP file, user must pass a list of files to check for the requisite v
...
The following properties are available in the `windows_msp` type.

##### `ensure`

Valid values: present, absent

Ensure the patch is applied. Removal/rollback not supported

Default value: present

#### Parameters

The following parameters are available in the `windows_msp` type.

##### `files`

Array of files or Hash of files+versions matching the state of the system after the patch has been applied

##### `version`

Version string to test the files listed in `files` for after the patch has been applied

##### `name`

namevar

Path to the MSP file to install
